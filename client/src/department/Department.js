...
const searchHandler=async()=>{
  ...
  if(searchVal===""){
    ...
    await axios.get(`/api/department/query/${searchVal}`).then((res) => {
...
const removeHandler=async(e)=>{
  await axios.get(`/api/department/delete/${e.target.id}`).then((res) => {
  if (res.data.code === 200) {
     alert('删除成功')
     getHttpData();
  }
  ...
  return (
    <ContentDiv >
      <div className='search'>
        ...
        <input type="text" value={searchVal} onChange={changeHandler} className='searchInput' placeholder='请输入部门名称'/>
